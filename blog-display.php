			...
			<div class="min-height-200px">
				<div class="page-header">
					<div class="row">
						...
						<div class="col-md-6 col-sm-12 text-right">
							<div class="dropdown">
								<a class="btn btn-primary " href="blog-add.php" role="button">
									Add
								</a>
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4">Blog table</h4>
					...
					<table class="table table-striped">
                        ...
                        <tbody>
                            <?php
                                ...
                                if(isset($_GET['did'])){
                                $deleteid=$_GET['did'];
                                $dq=mysqli_query($connection,"delete from tbl_blog where blog_id='{$deleteid}'");
                                if($dq)
                                {
                                    echo "<script><div class='alert alert-success' role='alert'>
									A simple success alert—check it out!</div></script>";
                                }
                                }
                                ?>
                                <?php
                              $sq=mysqli_query($connection,"select * from tbl_blog");
                              $count=mysqli_num_rows($sq);
                              echo "$count Records Found";
                              while($data = mysqli_fetch_array($sq))
                              {
                                  echo "<tr>";
                                  echo "<th scope='row'>{$data['blog_id']}</th>";
                                  echo "<td>{$data['blog_name']}</td>";
                                  echo "<td>{$data['blog_detail']}</td>";
                                  echo "<td>{$data['blog_date']}</td>";
                                //   echo "<td>{$data['blog_photo_path']}</td>"; 
								  echo "<td><a href='uploads/{$data['blog_photo_path']}' target='_blank'><img width='50px'src='uploads/{$data['blog_photo_path']}'></img></a></td>";             
                                  echo "<td><a href='blog-edit.php?eid={$data['blog_id']}'> Edit</a> |
                                  <a href='blog-display.php?did={$data['blog_id']}' onclick='return check();'> Delete</a></td>";
